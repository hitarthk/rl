...
def pretty_print(s_batch, q_values, a_batch, r_batch, sp_batch, done_mask_batch):
    for i in range(len(s_batch)):
        print(f'Current state: {s_batch[i] - 1}, action: {a_batch[i]} , Reward: {r_batch[i]}, q_value: {q_values[i][
            a_batch[i]]},next_state: {sp_batch[i] - 1}, done_mask: {done_mask_batch[i]}')
